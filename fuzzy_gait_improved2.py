...
#    rule8 = ctrl.Rule(x_h['high'] & x_k['low'] & x_s['S_SS'] & x_t['high'], phase['TSw'])
    ...
    if mode =='all':
        ...
        rule1 = ctrl.Rule(x_h['high'] & x_k['low']  & x_t['low'] & x_th['TO-HS'], phase['LR'])
        rule2 = ctrl.Rule(x_h['low'] | x_h['medium'] & x_k['low'] &  x_t['low'] & x_th['TO-HS'], phase['MSt'])
        rule3 = ctrl.Rule(x_h['medium'] & x_k['low'] & x_t['low'] & x_th['TO-HS'], phase['MSt'])
        #rule4 = ctrl.Rule(x_h['low'] & x_k['low'] & x_t['medium'], phase['TSt'])
        rule4 = ctrl.Rule(x_h['low'] & x_k['medium'] & x_t['medium'] & x_th['TO-HS'], phase['TSt'])
        #rule5 = ctrl.Rule(x_h['low'] & x_k['high'] & x_s['S_SS'] & x_t['medium'], phase['PSw'])
...
#        phase['ST'] = fuzz.gaussmf(phase.universe, 0.489,0.032)   
        phase['SW'] = fuzz.trimf(phase.universe,[1,1,2])
        phase['ST'] = fuzz.trimf(phase.universe,[2,2,3])    
        rule1 = ctrl.Rule(x_h['low']  | x_h['medium'] & ~x_th['TO-HS'], phase['ST'])
        rule2 = ctrl.Rule(x_h['high'] | x_h['medium']  & x_th['TO-HS'], phase['SW'])

        # control system
        phase_ctrl = ctrl.ControlSystem([rule1, rule2])
