...
for s in range(0,len(subjects)):
    vgrf, points =  vicon_reader_cluster.read_file(experiment, subjects[s])
    vgrfs.append(vgrf)
    a_r_knee_angle, a_l_knee_angle, r_hip_angle, l_hip_angle, r_ankle_angle, l_ankle_angle, trunk_angle = vicon_reader_cluster.kinematics(points)
    gait_cycle_l, gait_cycle_r, Fz_l, Fz_r = vicon_reader_cluster.gait_cycles(vgrf, False, False)
