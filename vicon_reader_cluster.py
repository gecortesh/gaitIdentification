...
def gait_cycles(vgrf, plot, full):
    ...
    if full == True:
        ...
        return full_gait_cycle, gait_cycle_l
    else:
        return gait_cycle_l, gait_cycle_r, Fz_l, Fz_r
...
def features_stance_swing(experiment, subjects):
    x = []
    y = []
    for s in range(0,len(subjects)):
        vgrf, points =  read_file(experiment, subjects[s])
        a_r_knee_angle, a_l_knee_angle, r_hip_angle, l_hip_angle, r_ankle_angle, l_ankle_angle, trunk_angle = kinematics(points)
        gait_cycle_l, gait_cycle_r = gait_cycles(vgrf, False, False)
        # swing rom
        r_knee_rom_sw = [] #np.zeros((len(gait_cycle_r)/2,1))
        l_knee_rom_sw = [] #np.zeros((len(gait_cycle_l)/2,1))
        r_hip_rom_sw = []
        l_hip_rom_sw = []
        r_ankle_rom_sw = []
        l_ankle_rom_sw = []
        ...
        for g in range (0,np.min((len(gait_cycle_l),len(gait_cycle_r)))-1):
            if (g%2)==0:
                r_knee_rom_sw.append(np.max(a_r_knee_angle[gait_cycle_r[g]:gait_cycle_r[g+1]])-np.min(a_r_knee_angle[gait_cycle_r[g]:gait_cycle_r[g+1]]))
                l_knee_rom_sw.append(np.max(a_l_knee_angle[gait_cycle_l[g]:gait_cycle_l[g+1]])-np.min(a_l_knee_angle[gait_cycle_l[g]:gait_cycle_l[g+1]]))
                r_hip_rom_sw.append(np.max(r_hip_angle[gait_cycle_r[g]:gait_cycle_r[g+1]])-np.min(r_hip_angle[gait_cycle_r[g]:gait_cycle_r[g+1]]))
                l_hip_rom_sw.append(np.max(l_hip_angle[gait_cycle_l[g]:gait_cycle_l[g+1]])-np.min(l_hip_angle[gait_cycle_l[g]:gait_cycle_l[g+1]]))
                r_ankle_rom_sw.append(np.max(r_ankle_angle[gait_cycle_r[g]:gait_cycle_r[g+1]])-np.min(r_ankle_angle[gait_cycle_r[g]:gait_cycle_r[g+1]]))
                l_ankle_rom_sw.append(np.max(l_ankle_angle[gait_cycle_l[g]:gait_cycle_l[g+1]])-np.min(l_ankle_angle[gait_cycle_l[g]:gait_cycle_l[g+1]]))
                trunk_rom_sw.append(np.max(trunk_angle[gait_cycle_r[g]:gait_cycle_r[g+1]])-np.min(trunk_angle[gait_cycle_r[g]:gait_cycle_r[g+1]]))
            else:
                r_knee_rom_st.append(np.max(a_r_knee_angle[gait_cycle_r[g]:gait_cycle_r[g+1]])-np.min(a_r_knee_angle[gait_cycle_r[g]:gait_cycle_r[g+1]]))
                l_knee_rom_st.append(np.max(a_l_knee_angle[gait_cycle_l[g]:gait_cycle_l[g+1]])-np.min(a_l_knee_angle[gait_cycle_l[g]:gait_cycle_l[g+1]]))
                r_hip_rom_st.append(np.max(r_hip_angle[gait_cycle_r[g]:gait_cycle_r[g+1]])-np.min(r_hip_angle[gait_cycle_r[g]:gait_cycle_r[g+1]]))
                l_hip_rom_st.append(np.max(l_hip_angle[gait_cycle_l[g]:gait_cycle_l[g+1]])-np.min(l_hip_angle[gait_cycle_l[g]:gait_cycle_l[g+1]]))
                r_ankle_rom_st.append(np.max(r_ankle_angle[gait_cycle_r[g]:gait_cycle_r[g+1]])-np.min(r_ankle_angle[gait_cycle_r[g]:gait_cycle_r[g+1]]))
                l_ankle_rom_st.append(np.max(l_ankle_angle[gait_cycle_l[g]:gait_cycle_l[g+1]])-np.min(l_ankle_angle[gait_cycle_l[g]:gait_cycle_l[g+1]]))
                trunk_rom_st.append(np.max(trunk_angle[gait_cycle_r[g]:gait_cycle_r[g+1]])-np.min(trunk_angle[gait_cycle_r[g]:gait_cycle_r[g+1]]))
        r = np.min((len(r_knee_rom_st),len(r_knee_rom_sw)))
        x.append(np.vstack([r_knee_rom_st[:r], l_knee_rom_st[:r], r_hip_rom_st[:r], l_hip_rom_st[:r], trunk_rom_st[:r], r_ankle_rom_st[:r], l_ankle_rom_st[:r],
                            r_knee_rom_sw[:r], l_knee_rom_sw[:r], r_hip_rom_sw[:r], l_hip_rom_sw[:r], trunk_rom_sw[:r], r_ankle_rom_sw[:r], l_ankle_rom_sw[:r]]))
        y.append(np.ones(len(x[s][0]))*s)
    x_vector = np.hstack((x))
    y_vector = np.hstack((y))
    return x_vector.T, y_vector